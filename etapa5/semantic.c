...
void checkDeclarations(ASTREE *root){
	...
	if(root->type == AST_VAR_DECL ||
		root->type == AST_VEC_DECL ||
		root->type == AST_FUNCT_DECL){

		if(root->symbol == 0) {
			printf("Line %d: Declaration is missing the identifier name.\n", root->lineNumber);
		} else {
			if (root->symbol->type != SYMBOL_IDENTIFIER)
				printf("Line %d: Symbol %s already defined.\n", root->lineNumber, root->symbol->value);
			if (root->type == AST_VAR_DECL) root->symbol->type = SYMBOL_VARIABLE_ID;
			if (root->type == AST_VEC_DECL) root->symbol->type = SYMBOL_VECTOR_ID;
			if (root->type == AST_FUNCT_DECL) root->symbol->type = SYMBOL_FUNCTION_ID;
		}
	}
